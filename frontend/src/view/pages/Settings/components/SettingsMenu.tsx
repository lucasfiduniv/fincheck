...
export function SettingsMenu({ items, activeKey, onSelect }: SettingsMenuProps) {
  return (
    <aside className="bg-white rounded-2xl border border-gray-200 p-3 xl:sticky xl:top-0">
      <span className="text-xs text-gray-500 uppercase tracking-[0.08em] px-2 block pt-1">
        Menu de configurações
      </span>

      <div className="mt-2 flex xl:block gap-2 overflow-x-auto pb-1 xl:space-y-2 xl:pb-0">
        {items.map((item) => (
          <button
            key={item.key}
            type="button"
            ...
            className={cn(
              'min-w-[220px] xl:min-w-0 w-full text-left rounded-xl border px-3 py-3 transition-colors',
              item.key === activeKey
